...
async function disegnaElenco() {
    ...
    todoList.forEach(
        function(item, index){
            ...
            elementoCheckbox.addEventListener('change', async function(event) {
                // cambio l'order del item dentro la lista
                item.order = event.target.checked ? 10 : 1;

                // aggiornaStatoTodo(idTodo, stato)
                // creaTodo(newTodo)
                // elminaTodo(newTodo)
                ...
                try {
                    await fetch(
                        `http://localhost:3000/todo/${item.id}`,
                        {
                            ...
                            body: JSON.stringify({stato: event.target.checked})
                        }
                    )
                } catch (err) {
                    console.error(err);
                    alert('Errore modificando la todo')
                }
...
const deleteItem = async (event) => {
    ...
    try {
        await fetch(
            `http://localhost:3000/todo/${idDaEliminare}`,
            {
                "method": "DELETE"
            }
        );
        ...
        console.error(err);
        alert('Errore cancellazione todo!');
    }
...
async function addItem () {
    ...
    try {
        ...
        console.error(err);
        alert('Errore creazione nuova todo!')
    }
